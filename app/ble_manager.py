...
class BleManager:
    ...
    async def scan(self, timeout: float = 6.0) -> list[dict]:
        ...
        found = []
        for d in devices:
            if str(d.name).startswith(DEVICE_NAME_PREFIXES):
    ...
    def _notification_handler(self, _char: BleakGATTCharacteristic, data: bytearray) -> None:
        ...
        if data.startswith(b"fen:") or data.startswith(b"Fen:") or data.startswith(b"FEN:"):
            # Prefix "fen:" / "fen: " strippen; trailing whitespace/\r\n weg.
            fen = bytes(data.split(b":", 1)[1]).decode("utf-8", errors="replace").strip()
            if fen:
                self._schedule(self._on_fen(fen))
        elif data.lower().startswith(b"get move"):

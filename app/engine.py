...
DEFAULT_SETTINGS = {
    "elo": 2000,       # UCI_Elo
    "analysis_depth": 15,  # diepte voor kandidaat-analyse
    "movetime": 1000,  # denktijd in ms voor de gespeelde zet
    "turn": "b",       # kleur die aan zet is als de arm om een zet vraagt
    "candidates": 5,   # aantal MultiPV-kandidaten
    "hash": 30720,     # MB hash-geheugen (30 GB)
    "threads": 20,     # aantal CPU-threads
}
...
class Engine:
    ...
    def update_settings(self, new_settings: dict) -> dict:
        """Neemt bekende sleutels over en past ze toe op de engine."""
        for key in DEFAULT_SETTINGS:
            if key in new_settings and new_settings[key] not in (None, ""):
                if key == "turn":
    ...
    def full_fen(self, fen_placement: str) -> str:
        """Vult de stukkenplaatsing van de arm aan tot een volledige FEN,
        zoals het voorbeeld doet met ' b - - 0 15'."""
        return f"{fen_placement} {self.settings['turn']} - - 0 15"
